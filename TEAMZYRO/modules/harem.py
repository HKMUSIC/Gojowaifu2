...
async def hmode_handler(client, message):
    ...
    if len(args) > 1:
        ...
        if rarity_input in rarity_map:
            ...
            c = await message.reply_text(f"Filter set to <b>{rarity_value}</b>", parse_mode=enums.ParseMode.HTML)
...
async def set_rarity_callback(client, cq):
    try:
        ...
        txt = (
            ...
            if rarity_value else "Filter cleared."
        )
